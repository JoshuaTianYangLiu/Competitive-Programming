...
void dfsrtou(int u,int p,ull hash=0){
    rtou[u]=hash;
    for(pi v:g[u]){
        if(v.first!=p)
            dfsrtou(v.first,u,hash+(v.second*depthpow[u]));
    }
}
void dfsutor(int u,int p,ull hash=0){
    utor[u]=hash;
    for(pi v:g[u]){
        if(v.first!=p)
            dfsutor(v.first,u,BB*(hash+v.second));
    }
}
int len(int u,int v){
...
ull calc(int u,int v){
    ...
    ull root_to_v = rtou[v]-rtou[llca];
